...
public class ConciertoDAO {
    ...
    public Concierto obtenerConciertoPorArtistaYFecha(String nombreArtistaBuscado, LocalDate fechaBuscada) {
        String query = String.format(
                "SELECT co.*, " +
                        "ar.%s, ar.%s, ar.%s, ar.%s, ar.%s, ar.%s, " +
                        "ed.%s, ed.%s, ed.%s, ed.%s, ed.%s, ed.%s, ed.%s, ed.%s " +
        ...
        try {
            ...
            preparedStatement.setDate(2, java.sql.Date.valueOf(fechaBuscada));
            resultSet = preparedStatement.executeQuery();
